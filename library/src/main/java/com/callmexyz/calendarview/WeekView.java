...
private CalendarView mCalendarView;
    public WeekView(Context context, int firstDayOfWeek,CalendarView calendarView) {
        super(context);
        mCalendarView = calendarView;
        if (firstDayOfWeek > Calendar.SATURDAY || firstDayOfWeek < Calendar.SUNDAY)
            firstDayOfWeek = 0;
        mFirstDayOfWeek = firstDayOfWeek;
        int[] names = new int[]{R.string.sunday_, R.string.monday_, R.string.tuesday_, R.string.wednesday_, R.string.thursday_, R.string.friday_, R.string.saturday_};
        for (int i = 0; i < 7; i++) {
            if (firstDayOfWeek > Calendar.SATURDAY) firstDayOfWeek = Calendar.SUNDAY;
